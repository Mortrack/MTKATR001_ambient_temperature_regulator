...
static uint16_t display_output[DISPLAY_5641AS_CHARACTERS_SIZE]; /**< @brief Global array variable used to hold the ASCII characters that are to be sent to the @ref display_5641as . */
	...
	switch (etx_ota_state)
	{
		...
		case ETX_OTA_STATE_START:
			if ((cmd->packet_type==ETX_OTA_PACKET_TYPE_CMD) && (cmd->cmd == ETX_OTA_CMD_START))
			{
				...
				/* Indicate to the user that an ETX OTA Transaction has started. */
				display_output[0] = 'E';
				display_output[1] = 'O';
			...
			if (header->packet_type == ETX_OTA_PACKET_TYPE_HEADER)
			{
				...
				switch (header->meta_data.payload_type)
				{
					case ETX_OTA_Application_Firmware_Image:
						...
						/* Indicate to the user that an Application Firmware Update will start. */
						display_output[0] = 'A';
						display_output[1] = 'F';
						display_output[2] = 'U';
						display_output[3] = 0; // This is a \c NULL value.
						set_5641as_display_output(display_output);
						break;
					case ETX_OTA_Bootloader_Firmware_Image:
						...
						/* Indicate to the user that a Bootloader Firmware Update will start. */
						display_output[0] = 'B';
						display_output[1] = 'F';
						display_output[2] = 'U';
						display_output[3] = 0; // This is a \c NULL value.
		...
		case ETX_OTA_STATE_DATA:
			...
			{
				/* We stop the 5641AS Driver Timer's Base generation in Interrupt Mode in order to guarantee a successful complete ETX OTA Transaction. */
...
static ETX_OTA_Status write_data_to_flash_app(uint8_t *data, uint16_t data_len, bool is_first_block)
{
	...
	/**	<b>Local variable word_data:</b> Array of 4 bytes (i.e., 1 word) initialized with the ones (i.e., 0xFF in each byte) to then overwrite them if needed with the remaining bytes of the last word from the Application Firmware Image. */
