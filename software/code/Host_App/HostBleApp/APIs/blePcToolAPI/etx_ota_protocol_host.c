 ...
 *          (connected to it via @ref Command_Line_Arguments::COMPORT_NUMBER ) to either indicate or identify in what
 *          part of the whole ETX OTA process is that external device currently at.
...
typedef enum
{
	ETX_OTA_STATE_IDLE    = 0U,   	//!< ETX OTA Idle State. @details This state stands for when the external device (connected to it via @ref Command_Line_Arguments::COMPORT_NUMBER ) is not in a ETX OTA Process.
	ETX_OTA_STATE_START   = 1U,   	//!< ETX OTA Start State. @details This state gives place when the external device (connected to it via @ref Command_Line_Arguments::COMPORT_NUMBER ) receives a Command Type Packet from the host right after leaving the ETX OTA Idle State. That external device will expect that Packet to contain the Start Command so that this external device starts an ETX OTA Process.
	ETX_OTA_STATE_HEADER  = 2U,   	//!< ETX OTA Header State. @details This state starts right after the Command Type Packet of the ETX OTA Start State is processed. In the ETX OTA Header State, the external device (connected to it via @ref Command_Line_Arguments::COMPORT_NUMBER ) will expect to receive a Header Type Packet in which this external device will obtain the size in bytes of the Payload to be received, its recorded 32-bits CRC and the Type of Payload to be received (i.e., @ref ETX_OTA_Payload_t ).
	ETX_OTA_STATE_DATA    = 3U,   	//!< ETX OTA Data State. @details This state starts right after the Header Type Packet of the ETX OTA Header State is processed. In the ETX OTA Data State, the external device (connected to it via @ref Command_Line_Arguments::COMPORT_NUMBER ) will expect to receive one or more Data Type Packets from which this external device will receive a Payload from the host. In the case of receiving a Firmware Update Image, it is also in this State where this external device will install that Firmware Image to itself.
	ETX_OTA_STATE_END     = 4U		//!< ETX OTA End State. @details This state starts right after the Data Type Packet(s) of the ETX OTA Data State is/are processed. In the ETX OTA End State, the external device (connected to it via @ref Command_Line_Arguments::COMPORT_NUMBER ) will expect to receive a Command Type Packet containing the End Command so that this external device confirms the conclusion of the ETX OTA Process.
} ETX_OTA_State;
...
typedef enum
{
    ETX_OTA_PACKET_TYPE_CMD       = 0U,   	//!< ETX OTA Command Type Packet. @details This Packet Type is expected to be send by the host to the external device (connected to it via @ref Command_Line_Arguments::COMPORT_NUMBER ) to request a certain ETX OTA Command to that external device (see @ref ETX_OTA_Command ).
    ETX_OTA_PACKET_TYPE_DATA      = 1U,   	//!< ETX OTA Data Type Packet. @details This Packet Type will contain either the full or a part/chunk of a Payload being send from the host to our external device (connected to it via @ref Command_Line_Arguments::COMPORT_NUMBER ).
    ETX_OTA_PACKET_TYPE_HEADER    = 2U,   	//!< ETX OTA Header Type Packet. @details This Packet Type will provide the size in bytes of the Payload that the external device (connected to it via @ref Command_Line_Arguments::COMPORT_NUMBER ) will receive, its recorded 32-bits CRC and the Type of Pyaload data to be received (i.e., @ref ETX_OTA_Payload_t ).
    ETX_OTA_PACKET_TYPE_RESPONSE  = 3U		//!< ETX OTA Response Type Packet. @details This Packet Type contains a response from the external device (connected to it via @ref Command_Line_Arguments::COMPORT_NUMBER ) that is given to the host to indicate to it whether or not that external device was able to successfully process the latest request or Packet from the host.
} ETX_OTA_Packet_t;
 ...
 *          (via @ref Command_Line_Arguments::COMPORT_NUMBER) whenever the host sends an ETX OTA Command Type Packet to
 *          that external device.
 */
typedef enum
{
    ETX_OTA_CMD_START = 0U,		    //!< ETX OTA Firmware Update Start Command. @details This command indicates to the MCU/MPU that the host is connected to (connected to to it via @ref Command_Line_Arguments::COMPORT_NUMBER ) to start an ETX OTA Process.
    ETX_OTA_CMD_END   = 1U,    		//!< ETX OTA Firmware Update End command. @details This command indicates to the MCU/MPU that the host is connected to (connected to to it via @ref Command_Line_Arguments::COMPORT_NUMBER ) to end the current ETX OTA Process.
    ETX_OTA_CMD_ABORT = 2U    		//!< ETX OTA Abort Command. @details This command is used by the host to request to the external device that the host is connected to (connected to to it via @ref Command_Line_Arguments::COMPORT_NUMBER ) to abort whatever ETX OTA Process that external device is working on. @note Unlike the other Commands, this one can be legally requested to the external device at any time and as many times as the host wants to.
} ETX_OTA_Command;

/**@brief	Response Status definitions available in the ETX OTA Protocol.
 *
 * @details	Whenever the host sends to the external device (connected to it via
 *          @ref Command_Line_Arguments::COMPORT_NUMBER ) a Packet, depending on whether that external device was able
 *          to successfully process the data of that Packet of not, this external device will respond back to the host
 *          with a Response Type Packet containing the value of a Response Status correspondingly.
 */
typedef enum
{
    ETX_OTA_ACK  = 0U,   		//!< Acknowledge (ACK) data byte used in an ETX OTA Response Type Packet to indicate to the host that the latest ETX OTA Packet has been processed successfully by the external device (connected to it via @ref Command_Line_Arguments::COMPORT_NUMBER ).
    ETX_OTA_NACK   = 1U   		//!< Not Acknowledge (NACK) data byte used in an ETX OTA Response Type Packet to indicate to the host that the latest ETX OTA Packet has not been processed successfully by the external device (connected to it via @ref Command_Line_Arguments::COMPORT_NUMBER ).
 ...
 *          device (connected to to it via @ref Command_Line_Arguments::COMPORT_NUMBER ) in a
 *          @ref ETX_OTA_PACKET_TYPE_HEADER Type Packet (i.e., in a Header Type Packet).
 */
typedef struct __attribute__ ((__packed__)) {
    uint32_t 	package_size;		//!< Total length/size in bytes of the data expected to be received by our external device (connected to it via @ref Command_Line_Arguments::COMPORT_NUMBER ) from the host via all the @ref ETX_OTA_PACKET_TYPE_DATA Type Packet(s) (i.e., in a Data Type Packet or Packets) to be received.
...
#define ETX_OTA_PACKET_MAX_SIZE 	    (ETX_OTA_DATA_MAX_SIZE + ETX_OTA_DATA_OVERHEAD)	                                                                    /**< @brief Maximum bytes that can exist in an ETX OTA Packet. */
...
/**@brief   Initializes the HM-10 Library, then configures the HM-10 Device that is to be used as a Bluetooth Dongle
 *          Device and finally connects it to the requested Remote Bluetooth Address.
 *
 ...
 */
static ETX_OTA_Status init_and_connect_ble_device(ETX_OTA_API_t *p_ETX_OTA_api);

/**@brief   Indicates whether the external device (connected to it via @ref Command_Line_Arguments::COMPORT_NUMBER )
 *          responded to our host machine with either an ETX OTA ACK or an ETX OTA NACK Response Status.
 *
 * @return                          \c true if there was both some data received from the external device (connected to
 *                                  it via @ref Command_Line_Arguments::COMPORT_NUMBER ) to our host machine and where
 *                                  also that data contained an ETX OTA Response Type Packet containing an ACK Response
 *                                  Status. Otherwise, \c false if either the expected data was not received from the
 *                                  external device to our host machine or the received data contained a NACK Response
 *                                  Status. In addition to this, regardless of the cases previously explained, if the
 *                                  CRC validation fails, then a \c false value will be given.
 *
 * @author 	EmbeTronicX (<a href=https://github.com/Embetronicx/STM32-Bootloader/tree/ETX_Bootloader_3.0>STM32-Bootloader GitHub Repository under ETX_Bootloader_3.0 branch</a>)
 * @author  César Miranda Meza (cmirandameza3@hotmail.com)
 * @date    November 25, 2023.
 */
static bool is_ack_resp_received();
 ...
 */
static ETX_OTA_Status send_etx_ota_abort();

/**@brief   Sends an ETX OTA Command Type Packet containing the Start Command in it to the external device (connected to
 *          it via @ref Command_Line_Arguments::COMPORT_NUMBER ).
 *
 * @note    Sending a Start Command to that external device will request to start an ETX OTA Protocol process.
 *
 * @retval  ETX_OTA_EC_OK
 ...
 */
static ETX_OTA_Status send_etx_ota_start();

/**@brief   Sends an ETX OTA Header Type Packet to the external device (connected to it via
 *          @ref Command_Line_Arguments::COMPORT_NUMBER ) that contains the general information of the Payload to be
 *          sent to that external device.
 *
 ...
 * @date    November 26, 2023.
 */
static ETX_OTA_Status send_etx_ota_header(header_data_t *etx_ota_header_info);

/**@brief   Sends an ETX OTA Data Type Packet to the external device (connected to it via
 *          @ref Command_Line_Arguments::COMPORT_NUMBER ) that contains some desired Payload Data.
 *
 ...
 * @date    November 26, 2023.
 */
static ETX_OTA_Status send_etx_ota_data(ETX_OTA_API_t *p_ETX_OTA_api, uint8_t *payload, uint16_t data_len);
 ...
 * @date    November 26, 2023.
 */
static ETX_OTA_Status send_etx_ota_end(ETX_OTA_API_t *p_ETX_OTA_api);
...
static ETX_OTA_Status init_and_connect_ble_device(ETX_OTA_API_t *p_ETX_OTA_api)
{
    ...
    ret = init_hm10_module(p_ETX_OTA_api->comport, p_ETX_OTA_api->send_packet_bytes_delay, p_ETX_OTA_api->teuniz_lib_poll_comport_delay, p_ETX_OTA_api->hm10_connect_to_address_timeout);
    if (ret != HM10_EC_OK)
    {
        return ETX_OTA_EC_BLE_INIT_ERR;
    }
    ...
    {
        return ETX_OTA_EC_BLE_AT_CMD_ERR;
    }

    /* Send Type Command to the HM-10 BT Device to set the Pin Code Mode. */
    // NOTE: As for now, the BT device will not be configured to connect with a pin.
    ret = set_hm10_pin_code_mode(HM10_Pin_Code_DISABLED);
    if (ret != HM10_EC_OK)
    {
        return ETX_OTA_EC_BLE_TYPE_CMD_ERR;
    }
    ...
    if (ret != HM10_EC_OK)
    {
        return ETX_OTA_EC_BLE_RESET_CMD_ERR;
    }

    /* Send Connect-To-Address Command to the HM-10 BT Device to start a Bluetooth Connection with the desired remote/external Bluetooth Device. */
    ret = connect_hm10_to_bt_address(HM10_BT_Static_MAC, p_ETX_OTA_api->bt_addr);
    if (ret != HM10_EC_OK)
    {
        return ETX_OTA_EC_BLE_CONN_CMD_ERR;
    }
...
static bool is_ack_resp_received()
{
    ...
    /** <b>Local variable ret:</b> Used to hold the exception code value returned by a @ref HM10_Status function type. */
    HM10_Status ret = get_hm10_ota_data(ETX_OTA_Packet_Buffer, sizeof(ETX_OTA_Response_Packet_t));
...
static ETX_OTA_Status send_etx_ota_abort()
{
    ...
    {
        if (send_hm10_ota_byte_of_data(ETX_OTA_Packet_Buffer[i]) != HM10_EC_OK)
        {
        ...
        ret = get_hm10_ota_data(tmp_ETX_OTA_Packet_Buffer, SIZE_OF_ETX_OTA_RESP_PACKT);
        if (ret == HM10_EC_OK)
        {
            /** <b>Local pointer etx_ota_resp:</b> Points to the data of the latest ETX OTA Packet but in @ref ETX_OTA_Response_Packet_t type. */
...
ETX_OTA_Status start_etx_ota_process(ETX_OTA_API_t *p_ETX_OTA_api)
{
    ...
    FILE *Fptr = NULL;
    /** <b>Local variable payload_size:</b> Will hold the size in bytes of the whole Payload. */
    uint32_t payload_size = 0;
    /** <b>Local variable ret:</b> Used to hold the exception code value returned by either a @ref ETX_OTA_Status or a @ref HM10_Status function type. */
    ETX_OTA_Status ret;
    ...
    /* Initialize the Bluetooth Dongle Device and start the desired Bluetooth Connected between that Device and the desired Remote Bluetooth Device. */
    if (payload_send_attempts == 0)
    {
        ret = init_and_connect_ble_device(p_ETX_OTA_api);
    ...
    ret = send_etx_ota_header(&etx_ota_header_info);
    ...
    for (uint32_t i=0; i<payload_size; )
    {
        ...
        ret = send_etx_ota_data(p_ETX_OTA_api, &PAYLOAD_CONTENT[i], size);
        if (ret != ETX_OTA_EC_OK)
        {
            return ret;
        }
    ...
    ret = send_etx_ota_end(p_ETX_OTA_api);
    if (ret != ETX_OTA_EC_OK)
    {
        return ret;
    }
